...
using System.Linq;
using ModestTree;
// Note, this is removed manually by GSN to support it's direct usage in games, This will have to be done again when updating zenject
// check git history of this commit to understand where else the compiler directive is commented off.
...
namespace Zenject
{
    public class SignalBus : ILateDisposable
    {
        ...
        // GSN Games - Commenting out as this hides Fire(object signal) method for signals that are not explicitly declared as objects.
        // commenting out doesn't break/change existing functionality and hence we only lose out this way of firing which is not used
        // or likely won't be used.
        ...
        public void Fire<T>(object signal)
        {
            InternalFire(typeof(T), signal, null, true);
        }
